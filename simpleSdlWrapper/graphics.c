...
int sgHasieratu()
{
  ...
  atexit(SDL_Quit);
  window = SDL_CreateWindow("Dino Drunker", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
  if (window == NULL)
  {
    fprintf(stderr, "Ezin lehioa sortu: %s\n", SDL_GetError());
    return -1;
  }
  /*icono = SDL_SHOWBMP(ICONO);
